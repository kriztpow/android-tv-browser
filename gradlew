...
if [ "$cygwin" = "true" ] || [ "$msys" = "true" ] ; then
    ...
    for i do
        ...
        CHECK2=`echo "$i"|egrep -c "^-"` ### Try to inplement an alternative to check for really long arguments that get truncated
        if [ $CHECK -ne 0 ] || [ $CHECK2 -gt 0 ] ; then
            addline=1
            CHECK=""
            for j in $i; do
            ### If the parameter does not contain a path (i.e., it is a program argument) then skip it
                if [ $CHECK -ne 0 ] ; then
                    break
                fi
                [ -e "$j" ] || continue
                jpath=`cygpath --path --ignore --mixed "$j"`
                if [ -z "$jpath" ] ; then
                    addline=0
                else
                    jpathcyg=`cygpath --path --ignore --mixed "$j"`
                    jpath=`printf "%q" "$jpathcyg"`
                fi
            done
            if [ $addline -ne 0 ] ; then
                ARGS="$ARGS $jpath"
            else
                ARGS="$ARGS $i"
            fi
